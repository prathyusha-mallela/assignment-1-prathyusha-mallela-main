 ...
 *
 * Name: Prathyusha Mallela
 * Email: mallelap@oregonstate.edu
 */
...
struct list* list_create() {
  struct list *listinit=(struct list *)malloc(sizeof(struct list));//initialising listinit dynamically
  listinit->head=NULL;
  return listinit;
  //return NULL;
}
...
void list_free(struct list* list) {
  ...
  if(l!=NULL)
  {
    ...
    //does not perform the operation if the list is already empty
...
void list_insert(struct list* list, void* val) {
  //initialising the link that is added to the beginning of the list
  struct link *firstnode=(struct link *)malloc(sizeof(struct link));
  //insert to the beginning of the node
  firstnode->val=val;
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
  int pos=0;
  for(struct link *l=list->head;l;l=l->next){
      if(*(int *)(l->val)==*(int *)(val)){
          return pos;//returns the position it the element is found
      }
      pos++;}
  return -1;
}
